"""===============================================================================
        ...
        FILE: /Users/nailbiter/Documents/forgithub/alex_leontiev_toolbox_python/alex_leontiev_toolbox_python/gdrive/spreadsheets.py
...
ORGANIZATION: 
     VERSION: ---
     CREATED: 2023-02-25T19:06:02.104460
    REVISION: ---
...
def get_creds(client_secret_file, token_file=".token.json", create_if_not_exist=False):
    ...
    if not creds or not creds.valid:
        if create_if_not_exist:
            ...
            raise Exception(f"provide valid credentials file: {token_file} given")
...
def upload_df_to_google_sheets(
    creds, df, spreadsheet_title, folder_id=None, sheet_name=None
):
    service = build("sheets", "v4", credentials=creds)
...
def _write_into_table(service, spreadsheet_id, df, sheet_name=None):
    ...
    ), f"too many columns: {len(list(df))}>{len(string.ascii_uppercase)}"
...
def _move_sheet_to_folder(creds, spreadsheet_id, folder_id):
    ...
    file_ = drive_service.files().get(fileId=file_id, fields="parents").execute()
